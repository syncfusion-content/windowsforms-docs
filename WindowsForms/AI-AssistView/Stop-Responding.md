...
`CancellationTokenSource` is created and assigned to `cts` when an AI request is started, and the resulting token is passed into the AI service call. The event handler cancels that token when the user clicks Stop.
...
| `NullReferenceException` when clicking Stop | `cts` was never assigned because the request was never started. | Guard the cancel call with a null check on `cts`, as shown in the example. |
