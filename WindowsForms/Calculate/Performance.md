...
If the application contains more nested if formulas for calculation, then setting `AllowShortCircuitIFs` property to `true` computes the formulas
quickly.
...
**ThrowCircularException** - Setting this property to `true` will throw an exception when the circular exception is encountered and also
