...
Public Class CustomRenderer
            ...
            For L As Integer = 1 To majorTicksCount
                ...
                If majorTickValue < LinearGauge.MaximumValue Then
                    ...
                    For S_Renamed As Integer = 1 To minor
                        ...
                        Graphics.DrawLine(minorTickPen, x, image(tickPositionMinor), x - LinearGauge.MinorTickHeight, image(tickPositionMinor))
                    Next s
                    temp1 = m_minorTicksPixels * L
                End If
            ...
            For Each ptrRange As LinearRange In Me.LinearGauge.Ranges
                ...
                If ptrRange.EndValue > ptrRange.StartValue AndAlso ptrRange.EndValue <= Me.LinearGauge.MaximumValue Then
                    If Me.LinearGauge.MinimumValue >= 0 AndAlso ptrRange.StartValue < 0 Then
                        Return
                    End If
                    Dim startValue As Single = image(ptrRange.StartValue)
                    Dim end As Single = image(ptrRange.EndValue)
                    If Me.LinearGauge.MinimumValue < 0 Then
        ...
        Public Sub DrawPointer(ByVal Graphics As System.Drawing.Graphics)
            ...
            Dim a As Integer = 0
            If Me.LinearGauge.MinimumValue < 0 Then
                a = CInt(Fix(Math.Ceiling((((Math.Abs(Me.LinearGauge.MinimumValue) + LinearGauge.Value) / image(LinearGauge.MajorDifference)) * m_minorTicksPixels))))
            Else
                a = CInt(Fix(Math.Ceiling(((LinearGauge.Value / image(LinearGauge.MajorDifference)) * m_minorTicksPixels))))
            End If
