---
layout: post
title: UI Automation with SfDataGrid for Syncfusion Essential WindowsForms
description: This section explains on how to test the UI of SfDataGrid projects.
...
<td>
Record and playback: The recorder identifies elements involved in an action, and the playback is processed based on the generated code via Microsoft Active Accessibility (MSAA).
</td>
</tr>
<tr>
<td>
...
The Coded UI support for SfDataGrid can be enabled using `AccessibilityEnabled` property.
...
Below are steps to create a `CodedUITest project` and test the `SfDataGrid` application.
...
![](UIAutomation_images/Coded UI_1.png)
...
![](UIAutomation_images/Coded UI_2.png)
...
![](UIAutomation_images/Coded UI_3.png)
...
6)Drag the Crosshairs on to the UI elements in `SfDataGrid` application and it shows the available properties of the inner UI elements in SfDataGrid. 
...
8)Once the record is completed, click the `GenerateCode` icon in CodedUITestBuilder for generate a test method. Then close the CodedUITestBuilder and see the generated code for cell value changed action as follows.
...
![](UIAutomation_images/Coded UI_7.png)

## QTP

You can refer the UFT/QTP document from [here](https://help.syncfusion.com/windowsforms/testing/uft/supported-controls-and-methods).
