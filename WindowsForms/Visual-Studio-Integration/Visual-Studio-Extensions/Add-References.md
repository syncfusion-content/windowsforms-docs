...
3. The Syncfusion Reference Manager Wizard that contains the list of Syncfusion WinForms controls that are loaded.
   ...
   **Assembly From:** Choose the assembly location, from where the assembly is added to the project.
   ...
   N> The GAC option will not be available, if you have selected WinForms (.NET 6.0, .NET 5.0 and .NET Core 3.1) application in Visual Studio 2019 or later.
   ...
   **Version:** Choose the build version to add the corresponding version assemblies to the project.
   ...
   N> Window Forms (.NET Core 3.1 and .NET 5.0) application in Visual Studio 2019 is supported from 18.2.0.44 version and .NET 6.0 application in Visual Studio 2022 is supported from 19.4.0.38. Version combobox is not visible for nuget option.
