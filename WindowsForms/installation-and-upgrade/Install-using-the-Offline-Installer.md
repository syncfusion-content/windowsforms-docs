---
layout: post
title: Essential Studio Offline Installer | WindowsForms | Syncfusion
...
The following procedure illustrates how to install Essential Studio Windows Forms platform.

1. Close all the running Visual Studio instances.

2. Double-click the Syncfusion Windows Forms platform installer file. The installer Wizard opens and extracts the package automatically.

   ![Installer extraction wizard](Platform_images/Step-by-Step-Installation_img1.png)

   N> The installer wizard extracts the syncfusionessentialwindowsforms_(version).exe dialog, displaying the unzip operation of the package.
...
7. The Completed screen is displayed once the Windows Forms platform is installed.
...
9. Click Finish. Syncfusion Windows Forms platform is installed in your system and the Syncfusion Essential Studio [Syncfusion Control Panel](http://help.syncfusion.com/common/essential-studio/utilities#dashboard ) is launched automatically.
...
2. The syncfusionessentialwindowsforms_(version).exe file is extracted into the Temp folder.
3. Run %temp%. The Temp folder will open. The syncfusionessentialwindowsforms_(version).exe file is available in one of the folders.
4. Copy the extracted syncfusionessentialwindowsforms_(version).exe file in local drive.
5. Cancel the Wizard.
6. Open the Command Prompt in administrator mode and pass the following arguments.
   ...
   **Example:** “D:\Temp\syncfusionessentialwindowsforms_x.x.x.x.exe” /Install silent /PIDKEY:“product unlock key” /log “C:\Temp\EssentialStudio_Platform.log” /InstallPath:C:\Syncfusion\x.x.x.x /InstallSamples:true /InstallAssemblies:true /UninstallExistAssemblies:true /InstallToolbox:true
...
1. When you do not have the extracted installer (syncfusionessentialwindowsforms_(version).exe) then follow the steps from 2 to 7.
...
3. The syncfusionessentialwindowsforms_(version).exe file is extracted into the Temp folder.
4. Run %temp%. The Temp folder will open. The syncfusionessentialwindowsforms_(version).exe file is available in one of the folders.
5. Copy the syncfusionessentialwindowsforms_(version).exe file in local drive. 
6. Cancel the Wizard.
7. Open the Command Prompt in administrator mode and pass the following arguments.
   
   **Arguments:** “Copied installer file path\syncfusionessentialwindowsforms_(version).exe” /uninstall silent 

   **Example:** “D:\Temp\syncfusionessentialwindowsforms_x.x.x.x.exe" /uninstall silent


8. Essential Studio for Windows Forms is uninstalled.
